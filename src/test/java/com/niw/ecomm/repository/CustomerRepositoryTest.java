...
class CustomerRepositoryTest {
    ...
    CustomerRepositoryTest(CustomerRepository customerRepository, OrderRepository orderRepository) {
        this.customerRepository = customerRepository;
        this.orderRepository = orderRepository;
    }

    private Customer savedCustomer(String name, String email) {
        return customerRepository.save(new Customer(name, email));
    }

    private Order orderWithItems(Customer customer, OrderStatus status, double... prices) {
        Order order = new Order(customer, status, LocalDate.now());

        for (double price : prices) {
            order.addItem(new OrderItem("PROD", 1, BigDecimal.valueOf(price)));
        }
     ...
     *
     * <p>Alice has 3 orders totalling €600.00; Bruno has 3 orders totalling €150.00.
    ...
    void returnsTopSpendersOrderedByTotal() {
        ...
        Customer alice = savedCustomer("Alice", "alice@test.com");
        orderWithItems(alice, OrderStatus.CONFIRMED, 100.0);
        orderWithItems(alice, OrderStatus.CONFIRMED, 200.0);
        orderWithItems(alice, OrderStatus.SHIPPED,   300.0);

        // Bruno: 3 orders → total = 50 + 50 + 50 = 150 (qualifies, but lower total)
        Customer bruno = savedCustomer("Bruno", "bruno@test.com");
        orderWithItems(bruno, OrderStatus.CONFIRMED, 50.0);
        orderWithItems(bruno, OrderStatus.CONFIRMED, 50.0);
        orderWithItems(bruno, OrderStatus.PENDING,   50.0);

        // Carla: 2 orders → does NOT qualify
        Customer carla = savedCustomer("Carla", "carla@test.com");
        orderWithItems(carla, OrderStatus.CONFIRMED, 999.0);
        orderWithItems(carla, OrderStatus.SHIPPED,   999.0);
    ...
    /**
     * When no customer has placed more than 2 orders the query must return an empty list.
     *
     * <p>Daniel has exactly 2 orders and therefore does not meet the {@code HAVING > 2}
     * threshold.
     */
    @Test
    @DisplayName("returns empty list when no customer has more than 2 orders")
    void returnsEmptyWhenNoQualifyingCustomers() {
        Customer daniel = savedCustomer("Daniel", "daniel@test.com");
        orderWithItems(daniel, OrderStatus.PENDING, 100.0);
        orderWithItems(daniel, OrderStatus.PENDING, 100.0);
