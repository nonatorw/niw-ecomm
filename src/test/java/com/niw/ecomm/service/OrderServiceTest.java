...
class OrderServiceTest {
    ...
    class CalculateTotal {
         ...
         *
         * <p>2 × €50.00 = <strong>€100.00</strong>.
         */
        @Test
         ...
         *
         * <p>100.00 + 30.00 + 60.00 = <strong>190.00</strong>.
         */
        @Test
        @DisplayName("returns sum of all items for multiple items")
        void multipleItems() {
            Order order = new Order(alice, OrderStatus.CONFIRMED, LocalDate.now());
            order.addItem(new OrderItem("PROD-001", 2, new BigDecimal("50.00")));  // 100.00
            order.addItem(new OrderItem("PROD-002", 1, new BigDecimal("30.00")));  //  30.00
            order.addItem(new OrderItem("PROD-003", 3, new BigDecimal("20.00")));  //  60.00
        ...
        /**
         * An item with quantity zero contributes {@code 0 × unitPrice = 0} to the total.
         */
        @Test
        @DisplayName("item with quantity zero contributes nothing")
        void itemWithZeroQuantity() {
            Order order = new Order(alice, OrderStatus.PENDING, LocalDate.now());
            order.addItem(new OrderItem("PROD-001", 0, new BigDecimal("99.99")));
    ...
    class GetOrdersByCustomer {
        ...
        /**
         * Only orders whose customer id matches the given argument are returned;
         * orders belonging to other customers are excluded.
         */
        @Test
        @DisplayName("returns only orders belonging to the given customer")
        void filtersByCustomer() {
            Order aliceOrder = new Order(alice, OrderStatus.PENDING, LocalDate.now());
            Order brunoOrder = new Order(bruno, OrderStatus.CONFIRMED, LocalDate.now());

            List<Order> result = service.getOrdersByCustomer(List.of(aliceOrder, brunoOrder), "1");
        ...
        /**
         * An empty list is returned when no order in the input matches the given customer id.
         */
        @Test
        @DisplayName("returns empty list when no orders match")
        void noMatch() {
            Order brunoOrder = new Order(bruno, OrderStatus.PENDING, LocalDate.now());

            List<Order> result = service.getOrdersByCustomer(List.of(brunoOrder), "999");
        ...
        /**
         * A {@code null} customer id cannot match any order, so the result is always empty.
         */
        @Test
    ...
    class GroupByStatus {
        ...
        /**
         * Orders are partitioned by their status; the resulting map contains an entry for
         * each status present in the input and no entry for statuses that are absent.
         */
        @Test
        @DisplayName("groups orders correctly by status")
        void groupsMixedOrders() {
            Order pending   = new Order(alice, OrderStatus.PENDING,   LocalDate.now());
            Order confirmed = new Order(alice, OrderStatus.CONFIRMED,  LocalDate.now());
            Order shipped   = new Order(bruno, OrderStatus.SHIPPED,    LocalDate.now());
            Order pending2  = new Order(bruno, OrderStatus.PENDING,    LocalDate.now());

            Map<OrderStatus, List<Order>> grouped =
                    service.groupByStatus(List.of(pending, confirmed, shipped, pending2));
        ...
        /**
         * Grouping an empty list produces an empty map — no null keys or empty-list entries.
         */
        @Test
        ...
        /**
         * When all orders share the same status the resulting map contains exactly one entry.
         */
        @Test
    ...
    class FindMostExpensive {
        ...
        void singleOrder() {
            Order order = new Order(alice, OrderStatus.PENDING, LocalDate.now());
            order.addItem(new OrderItem("PROD-001", 1, new BigDecimal("50.00")));
         ...
         *
         * <p>cheap = €10.00, expensive = 2 × €200.00 = <strong>€400.00</strong>, medium = €50.00.
         */
        @Test
        @DisplayName("returns order with highest total among multiple orders")
        void picksHighestTotal() {
            Order cheap     = new Order(alice, OrderStatus.CONFIRMED, LocalDate.now());
            Order expensive = new Order(alice, OrderStatus.CONFIRMED, LocalDate.now());
            Order medium    = new Order(bruno, OrderStatus.PENDING,   LocalDate.now());

            cheap.addItem(new OrderItem("PROD-A", 1, new BigDecimal("10.00")));
            expensive.addItem(new OrderItem("PROD-B", 2, new BigDecimal("200.00")));
            medium.addItem(new OrderItem("PROD-C", 1, new BigDecimal("50.00")));
        ...
        /**
         * When two orders have equal totals the first one encountered in the list is returned,
         * reflecting the stable {@code max} reduction used by the service.
         */
        @Test
        @DisplayName("returns first order when totals are tied")
        void tiedTotals() {
            Order first  = new Order(alice, OrderStatus.PENDING, LocalDate.now());
            Order second = new Order(bruno, OrderStatus.PENDING, LocalDate.now());

            first.addItem(new OrderItem("PROD-A", 1, new BigDecimal("100.00")));
            second.addItem(new OrderItem("PROD-B", 1, new BigDecimal("100.00")));
